...
from app.booking import booking_services
from app.booking.availability import availability_services
from app.booking.booking_type import booking_type_services
...
bp_booking = Blueprint('booking', __name__, url_prefix='/booking')
...
def get_availability_by_id(payload: dict, availability_id: int, session: Session):
    try:
        ...
        if availability.account_id != int(payload.get('sub')):
            raise PermissionError('You don\'t have any availability with id=: {}'.format(availability_id))
...
def get_booking_type_by_id(payload: dict, booking_type_id: int, session: Session):
    try:
        ...
        if booking_type.account_id != int(payload.get('sub')):
            raise PermissionError('You don\'t have any booking_type with id=: {}'.format(booking_type_id))
...
def me(payload: dict, session: Session):
    try:
        bookings = booking_services.me(sub=payload.get('sub'), session=session)
...
def get_booking_by_id(payload: dict, booking_id: int, session: Session) -> Response:
    try:
        booking = booking_services.find_by_id(booking_id=booking_id, session=session)
...
def create_booking(payload: dict, session: Session) -> Response:
    try:
        booking_services.validate_create(data=request.get_json(), session=session)
        booking = booking_services.create(sub=payload.get('sub'), data=request.get_json(), session=session)
        return Response(booking_schema.dumps(booking), content_type=Constants.CONTENT_TYPE_JSON, status=200)
    except Exception as err:
        raise err
...
def update_booking(payload: dict, booking_id, session: Session) -> Response:
    try:
        db_booking = booking_services.validate_update(sub=payload.get('sub'), booking_id=booking_id, data=request.get_json(), session=session)
        booking = booking_services.update(db_booking=db_booking, data=request.get_json(), session=session)
        return Response(booking_schema.dumps(booking), content_type=Constants.CONTENT_TYPE_JSON, status=200)
    except Exception as err:
        raise err
...
def delete_booking(payload: dict, booking_id: int, session: Session) -> Response:
    try:
        booking = booking_services.find_by_id(booking_id, session)
        if booking.account_id != int(payload.get('sub')):
            raise PermissionError('You don\'t have any booking with id=: {}'.format(booking_id))
        booking_services.delete(booking, session)
        return Response(status=204)
    except Exception as err:
        raise err
...
def cancel(payload: dict, booking_id: int, session: Session):
    try:
        ...
        if int(payload.get('sub')) == booking.account_id:
            ...
            booking_services.add(booking, session)
...
def confirm(payload: dict, booking_id: int, session: Session):
    try:
        ...
        if int(payload.get('sub')) == booking.account_id and booking.confirm_flag is None:
            ...
            booking_services.add(booking, session)
            booking_services.send_mail_confirm(booking)
            return Response(status=200)
        else:
            raise PermissionError('You don\'t have any booking with id=: {}'.format(booking_id))
    except Exception as err:
