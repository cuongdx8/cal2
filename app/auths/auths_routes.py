...
from app.auths import auths_service
...
def register(session: Session):
    try:
        ...
        auths_service.validate_register(data, session)
        account = users_schema.load(data)
        account.type = Constants.ACCOUNT_TYPE_LOCAL
        auths_service.register(account, session)
...
def login(session: Session):
    try:
        auths_service.validate_login(request.get_json())
        result = auths_service.login(request.get_json(), session)
...
def login_gg_callback(session: Session):
    try:
        ...
        account = auths_service.register(User(credentials=credentials, type=Constants.ACCOUNT_TYPE_GOOGLE), session)
        session.flush()
...
def facebook_login_callback(session: Session):
    try:
        ...
        account = auths_service.register(User(credentials=credentials, type=Constants.ACCOUNT_TYPE_FACEBOOK), session)
        session.flush()
        return Response(json.dumps({'token': jwt_utils.create_access_token(account)}), status=200)
    except Exception as err:
        raise err
...
def forgot_password(session):
    try:
        auths_service.validate_forgot_password(request)
        if request.method == Constants.GET_METHOD:
            auths_service.forgot_password(request.args.get('email'), session)
            return Response(status=200)
        else:
            try:
                data = request.get_json()
                sub = jwt_utils.get_payload(request.args.get('token')).get('sub')
                account = auths_service.change_password(sub, data.get('password'), session)
                return Response(json.dumps({'token': jwt_utils.create_access_token(account)}), status=200)
...
def verify_email(session: Session):
    try:
        ...
        try:
            auths_service.active_account(sub, session=session)
            return Response(status=200)
