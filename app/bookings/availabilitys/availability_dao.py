...
def add(availability: Availability, session: Session):
    if availability.default_flag:
        session.execute('UPDATE availabilitys set default_flag=false where default_flag=true')
    session.add(availability)


def set_default(availability_id: int, session: Session):
    session.execute('UPDATE availabilitys SET default_flag=false WHERE default_flag=true')
    session.execute(f'UPDATE availabilitys SET default_flag=true WHERE id={availability_id}')
